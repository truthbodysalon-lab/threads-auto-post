...
def main():
    ...
    print("\n=== ⑬ 「〜ているです」「〜ないではなく」型破綻（2026-07-21第3弾・実例4件）が検出されること ===")
    real_breaks = [
        ("口コミが増えない院の共通パターン、良いコンテンツを作れば売れると思っているです。\n来院後の体験設計を変えると、自然と口コミが増えていきます。\n継続して発信するが起点です。", "masa"),
        ("値引きで集客している院が値上げできない理由、良いコンテンツを作れば売れると思っているです。\nターゲットの悩みを言語化できると投稿が刺さるで選ばれる仕組みを先に作ると、値下げしなくても予約が入ります。", "masa"),
        ("「売上につながる投稿を頑張っているのに予約が来ない」の原因は広告費をコストだと思って一切使わないではなく数字を全く確認せず感覚で運用しているです。\n感情に訴えるストーリーを使うから見直してください。", "masa"),
        ("「LINE配信設計を頑張っているのに予約が来ない」の原因は投稿の目的がバラバラで一貫性がないではなく良いコンテンツを作れば売れると思っているです。\n他アカウントとのコラボを活用するから見直してください。", "masa"),
    ]
    for t, a in real_breaks:
        results.append(_assert_ng(t, a, f"実破綻例: {t[:28]}..."))
    # 「〜ているんです」は正しい日本語 → 誤検知しないこと
    results.append(_assert_ok("実は、体は毎日変わっているんです。\n小さな習慣の積み重ねが大事。", "truth",
                              "「ているんです」は正しい日本語（誤検知しない）"))
    results.append(_assert_ok("原因は年齢のではなく、习慣のせいでもない。\n「〜のではなく」は正規表現にマッチしないこと", "masa",
                              "「のではなく」は誤検知しない"))
