...
ACCOUNT_PROFILES = {
    ...
    "nagaoka": {
        "name": "@truth_nagaoka",
        "persona": "長岡市の整体師・まぁ先生（長岡特化アカウント）",
        "target": "長岡市在住・35〜55歳の慢性肩こり・頭痛に悩む方",
        "tone": "優しく寄り添う。1文1行。超短文。「 」で患者のセリフを引用。長岡の地域ネタ・季節感を盛り込む。共感→原因→解決の3ステップ。",
        "topics": ["肩こり", "頭痛", "姿勢", "水分不足", "食いしばり", "スマホ首", "根本改善", "長岡の季節と体", "セルフケア"],
        "cta_katakori": "https://beauty.hotpepper.jp/CSP/kr/reserve/?storeId=H000596246&couponId=CP00000008152494&add=0",
        "cta_zutsuu": "https://beauty.hotpepper.jp/CSP/kr/reserve/?storeId=H000596246&couponId=CP00000008707618&add=0",
        "profile_file": "SNS・Threads/truth_body_salon/まとめ（プロフィール・分析）.md",
        "themes": [
...
■ 絶対ルール
- 本文は3〜4行以内に収める（短くテンポよく）
- 本文の中に「長岡市」を必ず自然に入れる（ハッシュタグではなく本文中に）
- 長い解説は [COMMENT] で区切ってコメント1・2・3に分割する
...
- 猫背放置 → 内臓圧迫・呼吸が浅い・老け顔・{prof['cta_katakori'][:30]}...
- 眼精疲労放置 → めまい・吐き気スパイラル
