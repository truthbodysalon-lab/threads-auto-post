...
# 固定ベース素材（精度保証）
# 2026-07-24修正: 肩こり・頭痛専門院のため、専門外の症状（眠りの浅さ・体のだるさ等の
# 睡眠/全身倦怠系）を見出し(1文目の主症状)プールから除外し、肩こり・頭痛系で補充した。
_SYMPTOMS_BASE = [
    "肩こり", "頭痛", "首こり", "慢性的な肩の重さ", "頭の重さ",
    "朝の疲れ", "肩と首の張り",
    "こめかみの痛み", "緊張型頭痛", "肩甲骨まわりのこり"
]
# 「原因」は根本原因のみ。猫背・巻き肩は"症状/状態"なので原因に入れない
# （{cause}に入ると「巻き肩が浅いうちに」「巻き肩から整えると」等の変数事故になる。
#  2026-07-24修正: 実際は姿勢の歪み・首の前傾など根本原因を指す）。
_CAUSES_BASE = [
    "水分不足", "食いしばり", "姿勢の歪み", "デスクワーク",
    "スマホの見すぎ", "運動不足", "睡眠不足", "首の前傾", "寝姿勢の悪さ"
]
...
def load_truth_materials() -> dict:
    ...
    # 肩こり・頭痛が専門のため「呼吸」系・睡眠/全身倦怠系の症状は{symptom}プール
    # （見出し=1文目の主症状）から除外する。
    # 「呼吸」: 呼吸テーマ多用禁止・2026-07-24フィードバック「肩こり頭痛だから呼吸の内容は変更して」
    # 「眠りの浅さ/睡眠/だるさ/疲れやすさ/腰」: 専門外症状が見出しに出た実投稿指摘対応
    # （2026-07-24。ファイル抽出(お悩みまとめ.md等)由来の症状にも適用し、静的リストの
    #  削除だけでは防げない再混入を防ぐ。「眼精疲労」等コア症状との誤マッチが起きない
    #  よう「疲労」単体ではなく「疲れやすさ」のみを対象にする）。
    _OFFTOPIC = ("呼吸", "眠りの浅さ", "睡眠", "だるさ", "だるい", "疲れやすさ", "腰")
